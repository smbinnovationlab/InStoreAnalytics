...
class FaceRecServer():
    ...
    def __call__(self):
        ...
        while True:
            (rec_face_images,focus_index,is_force, customer_id) = self._queue.get(True)
                ...
                for index, (faceid, embeding) in enumerate(result):
                    ...
                    if need_record or index == focus_index:
                        ...
                        if index == focus_index:
                            if is_force:
                                async_focus_customer_with_record(customer_id, age_result[index], 1, 70,emotion_result[index],
                                                                 pic_id,current if need_record else self._recognized_faces[
                                                                     faceid])
                            else:
                                async_focus_customer_with_record(faceid, age_result[index], gender_result[index], 70,emotion_result[index],pic_id,
                                                             current if need_record else self._recognized_faces[faceid])
                        else:
                            async_add_visisit_record(faceid, age_result[index], gender_result[index], 70,emotion_result[index], pic_id,current)
